...
**Note**: Please don't run your parallelized jobs with Slurm under `~/ucph/hdir` as you might experience issues with Slurm incorrectly forwarding Kerberos tickets for the SMB protocol. We have suspicions that this is what causes the servers to go into a DRAIN state which requires manual intervention. Instead, run your jobs under `/projects/biostat01/people/abc1234` where `abc1234` is your KU user id.
